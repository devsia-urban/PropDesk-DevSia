...
/**
 * DAILY CRON: Evening Wrap-up (6 PM IST)
 * Sends a summary of pending/overdue follow-ups for the day.
 * Path: /api/cron/notifications/evening-summary
 */
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch overdue follow-ups (scheduled for today or earlier, but not closed)
    const { data: pendingFollowUps, error } = await supabaseAdmin
      ...
      `)
      .lte('follow_up_date', now.toISOString()) // Still pending/overdue up to right now
      .gte('follow_up_date', todayStartUTC.toISOString()) // Ignore very old ones to avoid spam, just today's pending
      .eq('is_deleted', false)
      .neq('status', 'closed')

    if (error) {
      console.error('[evening-summary] DB error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    if (!pendingFollowUps?.length) {
      return NextResponse.json({ message: 'No pending meetings' })
    }

    // Group follow-ups by agency, then by agent
    const agencyMap = new Map<string, typeof pendingFollowUps>()
    ...
    for (const [agencyId, followUps] of agencyMap) {
      ...
      for (const [agentId, agentFollowUps] of agentMap) {
        ...
        await sendUserNotification(agentId, agencyId, {
          ...
          message: `You have ${count} pending follow-up${count > 1 ? 's' : ''} today (${clientNames}${extra}). Please update them before logging off!`,
      ...
      await notifyAgencyAdmins(agencyId, {
        type: 'system',
        title: '🌙 Evening Wrap-up',
        message: `Team has ${totalMeetings} pending follow-up${totalMeetings > 1 ? 's' : ''} with ${totalAgents} agent${totalAgents > 1 ? 's' : ''} — ${breakdown}${extraAgents}.`,
    ...
    return NextResponse.json({
      ...
      message: `Sent ${processedCount} evening wrap-ups`
    })
  } catch (err) {
